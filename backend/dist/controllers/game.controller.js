...
exports.gameController = {
    ...
    get: (0, error_handler_1.asyncHandler)(async (req, res) => {
        ...
        if (!game) {
            throw new error_handler_1.AppError(404, 'Game not found');
        }
    ...
    create: (0, error_handler_1.asyncHandler)(async (req, res) => {
        ...
        if (!title) {
            throw new error_handler_1.AppError(400, 'Title is required');
        }
        if (!validation_1.validators.title(title)) {
            throw new error_handler_1.AppError(400, 'Title must be 3-100 characters');
        }
    ...
    update: (0, error_handler_1.asyncHandler)(async (req, res) => {
        if (!req.userId) {
            throw new error_handler_1.AppError(401, 'Not authenticated');
        }
        const { id } = req.params;
        if (!mongoose_1.default.Types.ObjectId.isValid(id)) {
            throw new error_handler_1.AppError(400, 'Invalid game id');
        }
        const body = typeof req.body === 'string'
            ? (() => {
                try {
                    ...
                    throw new error_handler_1.AppError(400, 'Invalid JSON body');
                }
            })()
            : req.body || {};
        ...
        if (game.creator.toString() !== req.userId) {
            throw new error_handler_1.AppError(403, 'Not authorized to update this game');
        }
        if (title !== undefined && typeof title !== 'string') {
            throw new error_handler_1.AppError(400, 'Invalid title');
        }
        if (title && !validation_1.validators.title(title)) {
            throw new error_handler_1.AppError(400, 'Invalid title');
        }
        if (settings !== undefined && (typeof settings !== 'object' || !settings)) {
            throw new error_handler_1.AppError(400, 'Invalid game settings');
        }
        ...
        if (category !== undefined) {
            if (typeof category !== 'string') {
                throw new error_handler_1.AppError(400, 'Invalid category');
            }
        ...
        if (tags !== undefined) {
            if (!Array.isArray(tags)) {
                throw new error_handler_1.AppError(400, 'Invalid tags');
            }
        ...
        if (thumbnail !== undefined) {
            if (thumbnail === null || thumbnail === '') {
                ...
                throw new error_handler_1.AppError(400, 'Invalid thumbnail');
            }
            else {
                const isDataUrl = thumbnail.startsWith('data:image/');
                if (!isDataUrl && !validation_1.validators.url(thumbnail)) {
                    throw new error_handler_1.AppError(400, 'Invalid thumbnail URL');
                }
    ...
    publish: (0, error_handler_1.asyncHandler)(async (req, res) => {
        if (!req.userId) {
            throw new error_handler_1.AppError(401, 'Not authenticated');
        }
        const { id } = req.params;
        ...
        if (game.creator.toString() !== req.userId) {
            throw new error_handler_1.AppError(403, 'Not authorized to publish this game');
        }
        game.published = true;
        await game.save();
        services_1.db.invalidateGameCache(id);
    ...
    delete: (0, error_handler_1.asyncHandler)(async (req, res) => {
        if (!req.userId) {
            throw new error_handler_1.AppError(401, 'Not authenticated');
        }
        const { id } = req.params;
        ...
        if (game.creator.toString() !== req.userId) {
            throw new error_handler_1.AppError(403, 'Not authorized to delete this game');
        }
    ...
    like: (0, error_handler_1.asyncHandler)(async (req, res) => {
        if (!req.userId) {
            throw new error_handler_1.AppError(401, 'Not authenticated');
        }
        const { id } = req.params;
        ...
        if (user && user.likedGames.includes(id)) {
            throw new error_handler_1.AppError(409, 'Already liked this game');
        }
    ...
    unlike: (0, error_handler_1.asyncHandler)(async (req, res) => {
        if (!req.userId) {
            throw new error_handler_1.AppError(401, 'Not authenticated');
        }
        const { id } = req.params;
    ...
    likeStatus: (0, error_handler_1.asyncHandler)(async (req, res) => {
        const { id } = req.params;
        let isLiked = false;
        if (req.userId) {
            const user = await models_1.User.findById(req.userId).select('likedGames').lean();
    ...
    getByCreator: (0, error_handler_1.asyncHandler)(async (req, res) => {
