...
exports.authController = {
    register: (0, error_handler_1.asyncHandler)(async (req, res) => {
        ...
        if (!username || !email || !password) {
            throw new error_handler_1.AppError(400, 'Missing required fields: username, email, password');
        }
        if (!validation_1.validators.username(username)) {
            throw new error_handler_1.AppError(400, 'Username must be 3-30 chars, letters, numbers, _, or -');
        }
        if (!validation_1.validators.email(email)) {
            throw new error_handler_1.AppError(400, 'Invalid email format');
        }
        if (!validation_1.validators.password(password)) {
            throw new error_handler_1.AppError(400, 'Password must be 8+ chars with uppercase, lowercase, and number');
        }
        const existing = await models_1.User.findOne({
            $or: [{ email: email.toLowerCase() }, { username: username.toLowerCase() }],
        });
        if (existing) {
            throw new error_handler_1.AppError(409, 'Username or email already registered');
        }
    ...
    login: (0, error_handler_1.asyncHandler)(async (req, res) => {
        const { email, password } = req.body;
        if (!email || !password) {
            throw new error_handler_1.AppError(400, 'Email and password required');
        }
    ...
    me: (0, error_handler_1.asyncHandler)(async (req, res) => {
        if (!req.userId) {
    ...
    getProfile: (0, error_handler_1.asyncHandler)(async (req, res) => {
        const { id } = req.params;
